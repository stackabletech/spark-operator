...
Because of `enableMonitoring: true`, all pods will be annotated for metric scraping. Metrics are retrieved from Spark's built in Prometheus Servlet. Only cluster-wide metrics are scraped currently (no application metrics).
